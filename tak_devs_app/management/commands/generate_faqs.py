...
class Command(BaseCommand):
    help = 'Generate default FAQs for TAK Kinship website'
    ...
    def handle(self, *args, **kwargs):
        ...
        if FAQ.objects.exists():
            overwrite = input("FAQs already exist. Do you want to replace them? (y/n): ").lower() == 'y'
            if overwrite:
        ...
        with transaction.atomic():
            ...
            count = len(faqs_data)
            self.stdout.write(self.style.SUCCESS(f'Successfully created {count} FAQs'))